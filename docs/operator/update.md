...
The **mobile preview** (`selfhelp-mobile-preview`) is an independently versioned
service, **provisioned by default with every install**, that lets a CMS admin
preview a page as it renders in the Expo mobile app, embedded in the page editor.
`instance install` resolves the newest core-compatible preview and bakes the
service into the instance automatically; because the preview is auxiliary, an
install where the registry has **no** compatible preview yet still succeeds (the
stack simply lands without it — add it later, see "Enabling the preview" below).
It is released on its own cadence in the registry (release kind
`selfhelp-mobile-preview-release`, listed under `registry.json#mobilePreview[]`)
and updated with the same **lightweight** path as the frontend — it is stateless,
so the manager:

- pulls **only** the mobile-preview image, then runs `docker compose up -d` so
  the backend rereads the stamped `SELFHELP_MOBILE_PREVIEW_VERSION`,
- takes a config snapshot (not a full database backup),
- runs **no** database migration and needs **no** maintenance window,
- leaves the core/backend image, the frontend image, and every volume untouched,
...
preview's in-container proxy to the backend over the internal Docker network.
Production adds a Traefik `Host(...) && PathPrefix(/mobile-preview)` route; local
mode uses the frontend's server-side `/mobile-preview/*` rewrite to reach the
same container. No extra public preview or backend port is opened.

**Enabling the preview on an existing instance.** New installs provision the
preview automatically, so most instances already have it. For one that does not —
installed before any compatible image was published, or one where the service was
removed — `update-mobile-preview` doubles as the **enable/bootstrap** path: it
resolves the newest compatible image and **creates** the container (no separate
"install preview" command). If nothing compatible is published yet, the dry run
reports a clear `blocked` plan ("installs automatically once one is available")
instead of a misleading "up to date", so you know it will land on its own once a
...
### Preview environment variables (version stamp + panel origin)

The preview is wired into the instance `.env` through two keys (generated by
`@shm/docker` `buildInstanceEnv`):

| Env var | Who sets it | Purpose |
|---------|-------------|---------|
| `SELFHELP_MOBILE_PREVIEW_VERSION` | **Manager** — re-asserted on every install/update; operators **cannot** override it | The version of the provisioned `selfhelp-mobile-preview` image. The CMS **System Maintenance → Mobile preview** row reads it (an unstamped value renders as a **"Not installed"** badge). Mirrors `SELFHELP_FRONTEND_VERSION`. |
| `NEXT_PUBLIC_MOBILE_PREVIEW_ORIGIN` | Manager (default `/mobile-preview`), **operator-overridable** | Where the page-editor **Mobile preview** panel embeds the iframe. The same-origin path is routed by Traefik in production and proxied by Next.js in local mode. Deliberately **not** manager-locked so a developer can repoint it at a live Expo dev server for fast refresh. |

You never set `SELFHELP_MOBILE_PREVIEW_VERSION` by hand on a managed instance —
it is stamped automatically by install / update / recreate / address-change
(sourced from the instance manifest's `versions.mobilePreview`). A
mobile-preview update refreshes the env-reading backend container after the
preview image swap, so the CMS row reports the new version immediately instead of
waiting for a later core/frontend update to restart Symfony.

**Dev / source checkout (no manager).** A bare `composer` / `npm run dev` checkout
has no manager to stamp these:

- the System-Maintenance **Mobile preview** row reads `SELFHELP_MOBILE_PREVIEW_VERSION`
  from your backend `.env` if you set it (otherwise it shows **"Not installed"**);
- the page-editor panel **auto-resolves with zero config** — it embeds an installed
  image at `/mobile-preview` when one answers `version.json`, else (in dev) falls
  back to the Expo dev server at `http://localhost:8081` for **live-reload**. Set
  `NEXT_PUBLIC_MOBILE_PREVIEW_ORIGIN` in the frontend `.env` to force a specific
  origin. Full precedence: frontend `docs/developer/mobile-preview-panel.md`.
