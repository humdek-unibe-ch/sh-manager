...
| `NEXT_PUBLIC_MOBILE_PREVIEW_ORIGIN` | Manager (default `/mobile-preview`), **operator-overridable** | Where the page-editor **Mobile preview** panel embeds the iframe. Set to the same-origin `/mobile-preview` Traefik route when the preview is provisioned. Deliberately **not** manager-locked so a developer can repoint it at a live Expo dev server for fast refresh. |
