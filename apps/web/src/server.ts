...
export function createManagerServer(options: ManagerServerOptions): ManagerServerHandle {
   ...
   * updates, backups, address/email/env changes, restarts — updates live
   * instead of waiting for the next poll. The browser keeps its polling as a
   * fallback, so a dropped stream never freezes the UI; this just removes the
   * lag. The session cookie authenticates the EventSource (same-origin GET, so
   * no CSRF token is needed).
   */
  function startEventStream(res: ServerResponse): void {
    const im = options.instanceManagement;
    res.statusCode = 200;
