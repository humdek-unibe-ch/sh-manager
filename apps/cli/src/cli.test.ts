...
async function makeDeps(): Promise<ActionDeps> {
  const fetcher = new FixtureFetcher({
    ...
    'selfhelp-core-1.5.0.json': await readExample('core-release.json'),
    'selfhelp-frontend-1.5.0.json': await readExample('frontend-release.json'),
  });
...
describe('CLI actions (offline)', () => {
  ...
  it('installs an instance from the signed fixture registry', async () => {
    ...
    expect(manifest.images.frontend).toContain('selfhelp-frontend:1.5.0');
